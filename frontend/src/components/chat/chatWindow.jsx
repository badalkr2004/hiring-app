...
const ChatWindow = ({ chatId }) => {
  ...
  const subscribeToMessages = () => {
    ...
    channel.bind("message:new", (message) => {
      ...
      const isOptimisticFromCurrentUser = optimisticMessagesRef.current.size > 0 && 
        message.senderId === userData?.id;

      if (isOptimisticFromCurrentUser) {
        // Find and replace the most recent optimistic message from current user
        setMessages((prev) => {
          const optimisticIndex = prev.findLastIndex(msg => 
            msg.isOptimistic && msg.senderId === userData?.id
          );
        ...
        // Clear one optimistic message reference
        const firstKey = optimisticMessagesRef.current.keys().next().value;
        if (firstKey) {
          optimisticMessagesRef.current.delete(firstKey);
        }
      } else {
  ...
  const sendMessage = async (e) => {
    ...
    const tempId = `temp-${timestamp}-${userData?.id}`;
